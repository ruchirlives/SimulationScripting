...
def summarize(text: str, *, client: OpenAI | None = None, max_words: int = 25) -> str:
    """Return a concise summary of the given text.

    Args:
        text: The text to summarize
        client: Optional OpenAI client instance
        max_words: Maximum number of words in the summary (default: 25)
    ...
    prompt = f"Please provide a summary of the following text in no more than {max_words} words:\n\n{text}"
...
def get_category(text: str, categories: str, prompt: str, *, client: OpenAI | None = None) -> str:
    ...
    full_prompt = (
        f"{prompt} \n"
        "Please classify the entry into one of the following categories, "
        "providing only the category by itself in the output. \n\n"
